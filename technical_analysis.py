...
class TechnicalAnalysis:
    ...
    def __init__(self):
        ...
        self.CACHE_DURATION_HISTORICAL = 3600  # Cache por 1 hora (em segundos)
        
    def get_historical_data(self, coin_id: str, days: int = 30, retry_count: int = 0) -> Optional[pd.DataFrame]:
        """
        Obtém dados históricos de preços do CoinGecko com cache e retry.
        """
        ...
        if retry_count > 0: # Adiciona um atraso apenas em retries ou se for a primeira chamada e o cache estiver vazio
        ...
        try:
            ...
            if http_err.response.status_code == 429:
                print(f"Erro 429 Too Many Requests para {coin_id}. Tentando novamente (tentativa {retry_count + 1})...")
                if retry_count < 3: # Limita o número de retries
                    return self.get_historical_data(coin_id, days, retry_count + 1) 
                else:
                    print(f"Máximo de retries atingido para {coin_id}.")
                    return None
            else:
                print(f"Erro HTTP ao obter dados históricos para {coin_id}: {http_err}")
                return None
