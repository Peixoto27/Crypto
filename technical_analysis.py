...
class TechnicalAnalysis:
    ...
    def get_historical_data(self, coin_id: str, days: int = 30) -> Optional[pd.DataFrame]:
        """
        Obtém dados históricos de preços do CoinGecko
        
        Args:
            coin_id: ID da moeda no CoinGecko (ex: 'bitcoin')
            days: Número de dias de histórico (máximo 365 para API gratuita)
            
        Returns:
            DataFrame com colunas: timestamp, price, market_cap, volume
        """
        try:
